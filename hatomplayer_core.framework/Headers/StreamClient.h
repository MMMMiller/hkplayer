...
@property(nonatomic, copy) NSString *broadcastIndexCodes;
@property (nonatomic, strong) NSDictionary *broadcastHeaders;
...
/// 设置广播参数
/// @param path 广播url
/// @param indexCode 广播通道号
/// @param headers 请求参数
- (void)setBroadcastDataSource:(NSString *)path
...
/// 发送编码后到数据
/// @param broadcastData 编码后的广播数据
/// @param dataLenght 编码后的广播数据长度
- (int)sendBroadcastData:(char *)broadcastData
              dataLenght:(int)dataLenght;
